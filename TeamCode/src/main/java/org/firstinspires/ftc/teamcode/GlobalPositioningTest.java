...
public class GlobalPositioningTest extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            if (gamepad1.right_trigger > 0.05 && gamepad1.right_trigger < 0.75) { // if precision mode is on (the right
                                                                                  // trigger is pulled down to some
                                                                                  // degree)
                speedMultiplier = 1 - gamepad1.right_trigger;
                telemetry.addData("Precise Mode", "On");
            } else if (gamepad1.right_trigger >= 0.75) { // also if precision mode is on, but it's fully or almost fully
                                                         // pu
                speedMultiplier = 0.25;
                telemetry.addData("Precise Mode", "On");
            } else { // if precise mode is off
                telemetry.addData("Precise Mode", "Off"); // if precision mode is off, and the robot will slowly
                                                          // accelerate
             ...
             * 
             * if (gamepad1.left_stick_x != 0 || gamepad1.left_stick_y != 0) { // if the
             * joystick is moved
             ...
             * }
             * }
             * Overrode value while we test out PID, PID should replace
